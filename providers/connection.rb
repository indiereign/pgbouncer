...
action :start do
  service "pgbouncer-#{new_resource.name}-start" do
    service_name "pgbouncer-#{new_resource.name}" # this is to eliminate warnings around http://tickets.opscode.com/browse/CHEF-3694
...
action :restart do
  service "pgbouncer-#{new_resource.name}-restart" do
    service_name "pgbouncer-#{new_resource.name}" # this is to eliminate warnings around http://tickets.opscode.com/browse/CHEF-3694
    provider Chef::Provider::Service::Upstart
...
action :stop do
  service "pgbouncer-#{new_resource.name}-stop" do
    service_name "pgbouncer-#{new_resource.name}" # this is to eliminate warnings around http://tickets.opscode.com/browse/CHEF-3694
    provider Chef::Provider::Service::Upstart
    action :stop
  end
...
action :setup do
  ...
  service "pgbouncer-#{new_resource.name}" do
    provider Chef::Provider::Service::Upstart
  ...
  [
   ...
   ::File.expand_path(::File.join(new_resource.socket_dir, new_resource.name)),
   '/etc/pgbouncer'
  ].each do |dir|
    directory dir do
      action :create
  ...
  # add the name of the resource as a possible property
  properties["name"] = new_resource.name


  # build the userlist, pgbouncer.ini, upstart conf and logrotate.d templates
  {
    "/etc/pgbouncer/userlist-#{new_resource.name}.txt" => 'etc/pgbouncer/userlist.txt.erb',
    "/etc/pgbouncer/pgbouncer-#{new_resource.name}.ini" => 'etc/pgbouncer/pgbouncer.ini.erb',
    "/etc/init/pgbouncer-#{new_resource.name}.conf" => 'etc/init/pgbouncer.conf.erb',
    "/etc/logrotate.d/pgbouncer-#{new_resource.name}" => 'etc/logrotate.d/pgbouncer-logrotate.d.erb'
    ...
    template destination_file do
      ...
      group new_resource.group
      mode 0644
      notifies :restart, "service[pgbouncer-#{new_resource.name}]"
...
action :teardown do
  ...
  { "/etc/pgbouncer/userlist-#{new_resource.name}.txt" => 'etc/pgbouncer/userlist.txt.erb',
    "/etc/pgbouncer/pgbouncer-#{new_resource.name}.ini" => 'etc/pgbouncer/pgbouncer.ini.erb',
    "/etc/init/pgbouncer-#{new_resource.name}.conf" => 'etc/pgbouncer/pgbouncer.conf',
    "/etc/logrotate.d/pgbouncer-#{new_resource.name}" => 'etc/logrotate.d/pgbouncer-logrotate.d'
